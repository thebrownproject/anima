...
## [2026-02-07 19:45] Session 126

**Branch:** main | **Git:** clean (after commit+push)

### What Happened
- **Task m7b.2.4 (golden checkpoint) — CLOSED**
  - Wired `checkAndUpdate()` into `bridge/src/proxy.ts:80-85` (best-effort, try/catch, doesn't block connections)
  - Fixed ESM mock issue in `bridge/tests/proxy.test.ts` — `vi.mock()` at module level instead of direct property assignment (ESM modules are immutable)
  - Added 4 updater integration tests: new connection calls update, proceeds on success, proceeds on failure, skips on reuse
  - Created `docs/ops/golden-checkpoint.md` (103 lines) — documents bootstrap flow, lazy update, server startup, sleep/wake, file layout
  - Fixed DRY violation: extracted `deployCode()` as export from `bootstrap.ts`, updater.ts now imports it (Inspector caught this)
  - 79/79 tests passing at close
- **Task m7b.2.8 (E2E smoke test) — IN PROGRESS, code complete**
  - Created `bridge/tests/e2e-echo.test.ts` (354 lines) — full round-trip smoke test
  - 5 E2E tests: mission echo, canvas_interaction echo, unknown type error, latency < 500ms, sequential messages
  - Mock Sprite server mimics TCP Proxy handshake + gateway.py echo behavior
  - 84/84 tests passing
  - Deployment steps remain (Fly.io deploy, DNS config, browser test, sleep/wake test with real Sprite)
- **Orchestrated mission mode** used — Scout → Worker → Inspector cycle for each task
  - Worker agents wrote initial code, HOUSTON fixed ESM mock issues and DRY violation post-inspection

### Decisions Made
1. **`checkAndUpdate()` called in `proxy.ts:ensureSpriteConnection()`** — runs before TCP Proxy connection, best-effort with try/catch. Every new connection checks for updates, reused connections skip.
2. **`deployCode()` shared between bootstrap and updater** — single source of truth for file list. Adding a new .py file only requires updating bootstrap.ts.
3. **`vi.mock()` for ESM modules** — can't assign properties on ESM module objects. Must use `vi.mock()` at module level, then `vi.mocked()` to access mock.

### Gotchas
- ESM module imports are immutable — `vi.spyOn(module, 'fn')` works but `module.fn = vi.fn()` throws "Cannot set property which has only a getter"
- Worker agents sometimes use `vi.spyOn` + direct assignment pattern that fails with ESM. Always use `vi.mock()` for mocking ESM module exports in vitest.

### In Progress
- m7b.2.8 code is done (E2E test written, 84/84 passing)
- Deployment remains: Fly.io deploy, `ws.stackdocs.io` DNS, sleep/wake test with real Sprite, browser test

### Next Action
- Deploy Bridge to Fly.io (needs env vars: SPRITES_TOKEN, CLERK_JWT_KEY, CLERK_SECRET_KEY, SUPABASE_URL, SUPABASE_SERVICE_KEY)
- Configure `ws.stackdocs.io` DNS CNAME
- Test sleep/wake cycle with real Sprite (35s wait)
- Close m7b.2.8 and Phase 1

---

## [2026-02-07 22:20] Session 127

**Branch:** main | **Git:** uncommitted changes

### What Happened
- **Closed m7b.2.8 (E2E smoke test) and Phase 1 (m7b.2) — all 9 tasks complete**
- Deployed Bridge to Fly.io: `flyctl apps create stackdocs-bridge`, `flyctl deploy` from `bridge/`
- Set secrets: `CLERK_SECRET_KEY`, `SUPABASE_URL`, `SUPABASE_SERVICE_KEY`, `SPRITES_TOKEN`
- Configured DNS: CNAME `ws` → `r1y2qx8.stackdocs-bridge.fly.dev` in Vercel DNS
- TLS cert auto-issued by Let's Encrypt, verified at `wss://ws.stackdocs.io`
- Created `bridge/scripts/smoke-test.mjs` — production WS connection test (auth timeout verified)
- **Security fix**: Removed API key injection from `provisioning.ts:getEnvVarsForSprite()` — prompt injection risk. Created m7b.3.6 to design API proxy through Bridge instead.
- Updated `provisioning.test.ts` to match (removed ANTHROPIC/MISTRAL env var setup)
- Scaled to 1 machine (destroyed second), set `min_machines_running = 0` in fly.toml (~$2/mo)
- Saved `SPRITES_TOKEN` to `bridge/.env` (gitignored) for future sessions
- Installed `flyctl` via brew, authenticated as fraserbrown@live.com

### Decisions Made
1. **API keys NOT injected into Sprites** — prompt injection could exfiltrate via Bash/WebFetch. Tracked as m7b.3.6 (blocks m7b.3.3 agent runtime). Will proxy API calls through Bridge.
2. **1 machine, min 0** — 2nd machine unnecessary for pre-launch MVP. Auto-starts on request, ~1-2s cold start.
3. **No CI/CD yet** — manual `flyctl deploy` for now. GitHub Actions later when needed.
4. **Sleep/wake E2E deferred** — needs real Sprite server running (Phase 2 work).

### Next Action
- Phases 2 (Sprite Runtime) + 3 (Canvas UI) are unblocked and can run in parallel
- Phase 2 first tasks: m7b.3.6 (API key proxy design), m7b.3.1 (SQLite database layer)
- Phase 3 first tasks: m7b.4.1 (WebSocket connection manager), m7b.4.2 (React Flow canvas)

---

## [2026-02-08 17:15] Session 135

**Branch:** main | **Git:** uncommitted

### What Happened
- **Canvas UI foundation (m7b.4.2 in progress)** — Installed `@xyflow/react`, created `frontend/components/canvas/stack-canvas.tsx` (React Flow wrapper with pan/zoom/grid/snap), `canvas-card.tsx` (custom node with title bar drag handle, NodeResizer, close button), `card-renderer.tsx` (renders Block[] with inline placeholder blocks for all 8 types).

- **Test-chat page upgraded** — `frontend/app/(app)/test-chat/page.tsx` now split into chat panel (left, w-96) + Canvas panel (right, flex-1). When agent sends `canvas_update` messages, cards render live on the Canvas. Verified working end-to-end with agent creating an "Invoice Processing Information" card with heading, text, and key-value blocks.

- **Runtime persistence across reconnections** — Moved `AgentRuntime` from connection-scoped to server-scoped in `sprite/src/server.py`. Added `update_send_fn()` to `runtime.py`. Updated `gateway.py` to accept external runtime. Now conversation context survives TCP disconnects (sleep/wake, page reload). Verified: agent remembered "42" across disconnect/reconnect with same session_id.

- **Canvas tool descriptions fix (stackdocs-2d5)** — Expanded `create_card` and `update_card` tool descriptions in `sprite/src/agents/shared/canvas_tools.py` with full block schema, field names, and gotchas (content not text, separator not divider, don't provide id). Should eliminate the 8-9 retry pattern.

- **Closed beads** — m7b.6.4 (multi-turn conversation, carry-forward from session 134), stackdocs-2d5 (tool descriptions bug).

- **Created `/sprite-deploy` skill** — `.claude/skills/sprite-deploy/SKILL.md` — 5-step playbook for deploying sprite code to a VM (deploy via FS API, chown, kill, start via exec WS, verify).

- **Created `bridge/scripts/test-resume.ts`** — Automated test: connect → tell agent a number → disconnect → reconnect → ask for number back. Confirms runtime persistence works.

### Decisions Made
1. **Canvas card NOT using shadcn Card wrapper** — React Flow custom nodes need full layout control (drag handle, NodeResizer). Using same CSS classes as shadcn Card for visual consistency instead.
2. **Server-scoped runtime over connection-scoped** — Same pattern as Database. Runtime (with SDK client) survives TCP reconnects. Gateway accepts optional runtime param (server-scoped for prod, self-created for tests).
3. **Inline placeholder blocks in card-renderer** — All 8 block types rendered inline for now. Will be extracted to individual shadcn-based components in m7b.4.3.

### Gotchas
- `sprite exec` CLI doesn't support `max_run_after_disconnect` — must use WS exec API for persistent server processes
- `pgrep -f "python.*src.server"` can match itself during kill — check output carefully
- First resume test after cold start fails (no prior context) — expected, works on warm runtime

### In Progress
- m7b.4.2 (React Flow canvas) — core files created, test-chat integration working. Not formally closed yet as it needs the page rewrite and store integration.
- m7b.4 (Phase 3 Canvas UI) — feature activated, 2/8 tasks closed, 6 remaining.

### Next Action
**Brainstorm session** — User wants to rethink Canvas approach:
1. Responsive grid canvas instead of free-form React Flow (dashboard-style cards)
2. Proactive agent canvas usage (agent surfaces docs/data without being asked)
3. Document intelligence loop (sort → extract → present → user corrects → agent learns)
This is a design-level discussion that should happen before continuing m7b.4.3+ tasks. Use `/exploration brainstorm`.
